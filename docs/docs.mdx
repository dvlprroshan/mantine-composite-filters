...
Build powerful filter interfaces in minutes. `MultiFiltersInput` brings LinkedIn-style composite filtering to your Mantine apps - complete with type-safe definitions, smart operators, presets, and full keyboard support.
...
```tsx
<MultiFiltersInput classNames={{ container: 'my-filter' }} />
```
...
```tsx
<MultiFiltersInput storageKeyPrefix="users-page" />
<MultiFiltersInput storageKeyPrefix="orders-page" />
```
